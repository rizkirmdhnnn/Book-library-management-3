...
namespace Book_library_management_3.Models.Repository
{
    ...
    public class HistoryRepository
    {
        ...
        public List<History> getHistoryByUsername(History history)
        {
            ...
            try
            {
                string sql = @"SELECT history_id, transaction_id, username, books.title,date,status FROM history JOIN books ON history.isbn = books.isbn WHERE username LIKE @username";
                using (SQLiteCommand cmd = new SQLiteCommand(sql, _connection))
                {
